...
/-!
The tensor noncentral delta residues, expressed as actual first- and second-root tensor modes
...
theorem sum_tensorRootSummand_coeff (w : K) (β : Lattice) (f : Space K) (i : ℤ) :
    ∑ j : Fin 3, (tensorRootSummand w β j f).coeff (-i) =
      (12 : K) • tensorRootMode w β i f := by
  have h : tensorRootMode w β i f =
      (1 / 12 : K) • ∑ j : Fin 3, (tensorRootSummand w β j f).coeff (-i) := by
    change (tensorRootField w β f).coeff (-i) = _
...
theorem tensorFirstRoot_delta_sum_fusion (w : K) (hw : w ^ 4 - w ^ 2 + 1 = 0)
    (p q : ℕ) (β : Lattice)
    (hfusion : (coxeter^[p]) (simpleRoot 0) + simpleRoot 0 = (coxeter^[q]) β)
    (f : Space K) (a b : ℤ) :
    (1 / 144 : K) • ∑ j : Fin 3,
  ...
  simp_rw [tensorFirstRoot_delta_fusion w hw p q β hfusion]
  rw [← Finset.smul_sum, sum_tensorRootSummand_coeff, smul_smul, smul_smul]
  congr 1
  ring

theorem tensorFirstRoot_delta_four (w : K) (hw : w ^ 4 - w ^ 2 + 1 = 0)
    (f : Space K) (a b : ℤ) :
    (1 / 144 : K) • ∑ j : Fin 3,
      contract (fun n : ℤ => MvPolynomial.C (w ^ (-4 * n)))
        (tensorNormalProduct w (simpleRoot 0) (simpleRoot 0) j j f) (-a) (-b) =
      (w ^ (-2 * a + 2 * b) / 12) • tensorRootMode w (simpleRoot 0) (a + b) f := by
  have h := tensorFirstRoot_delta_sum_fusion w hw 4 2 (simpleRoot 0)
    RootData.first_fusion_four f a b
  norm_num only [Nat.cast_ofNat] at h
  exact h

theorem tensorFirstRoot_delta_eight (w : K) (hw : w ^ 4 - w ^ 2 + 1 = 0)
    (f : Space K) (a b : ℤ) :
    (1 / 144 : K) • ∑ j : Fin 3,
      contract (fun n : ℤ => MvPolynomial.C (w ^ (-8 * n)))
        (tensorNormalProduct w (simpleRoot 0) (simpleRoot 0) j j f) (-a) (-b) =
      (w ^ (2 * a - 2 * b) / 12) • tensorRootMode w (simpleRoot 0) (a + b) f := by
  have h := tensorFirstRoot_delta_sum_fusion w hw 8 10 (simpleRoot 0)
...
theorem tensorFirstRoot_delta_five (w : K) (hw : w ^ 4 - w ^ 2 + 1 = 0)
    (f : Space K) (a b : ℤ) :
    (1 / 144 : K) • ∑ j : Fin 3,
      contract (fun n : ℤ => MvPolynomial.C (w ^ (-5 * n)))
        (tensorNormalProduct w (simpleRoot 0) (simpleRoot 0) j j f) (-a) (-b) =
      (w ^ (4 * a + 9 * b) / 12) • tensorRootMode w (simpleRoot 1) (a + b) f := by
  have h := tensorFirstRoot_delta_sum_fusion w hw 5 9 (simpleRoot 1)
    RootData.first_fusion_five f a b
  norm_num only [Nat.cast_ofNat] at h
  exact h

theorem tensorFirstRoot_delta_seven (w : K) (hw : w ^ 4 - w ^ 2 + 1 = 0)
    (f : Space K) (a b : ℤ) :
    (1 / 144 : K) • ∑ j : Fin 3,
      contract (fun n : ℤ => MvPolynomial.C (w ^ (-7 * n)))
        (tensorNormalProduct w (simpleRoot 0) (simpleRoot 0) j j f) (-a) (-b) =
      (w ^ (9 * a + 4 * b) / 12) • tensorRootMode w (simpleRoot 1) (a + b) f := by
  have h := tensorFirstRoot_delta_sum_fusion w hw 7 4 (simpleRoot 1)
